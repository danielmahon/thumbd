	...
	*/
	...
	async.waterfall([
		...
		function(uploadedFiles, done) {
			//format output
			var endpoint = utils.s3().endpoint;
			job.output = [];
			_.each(uploadedFiles, function(file) {
				job.output.push({
					name: file.name,
					filename: file.path.split('/').pop(),
					url: 'https://' + endpoint + '/' + job.bucket + file.path,
					path: file.path
				});
			});
...
Worker.prototype._createThumbnails = function(localPaths, job, callback) {
	...
	job.descriptions.forEach(function(description) {
		work.push(function(done) {
			...
			// validate suffix
			description.suffix = description.suffix === undefined ? description.name : description.suffix;
...
Worker.prototype._thumbnailKey = function(prefix, suffix, format) {
	return prefix + (suffix ? '_' + suffix : '') + '.' + (format || 'jpg');
};
...
Worker.prototype._notify = function(job, cb) {
  ...
  if (!job.notify) return cb();
	...
	var sqsQueueUrl = this.sqs.endpoint.protocol + '//' + this.sqs.endpoint.hostname + '/' + job.notify;
	
	this.sqs.sendMessage({QueueUrl: sqsQueueUrl, MessageBody: JSON.stringify(job)}, function (err, result) {
		if (!err) _this.logger.info('notified:', job.notify);
		if (cb) cb(err, result);
	});
}
